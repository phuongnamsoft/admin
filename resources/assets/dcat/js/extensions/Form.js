...
class Form {
    ...
    init() {
        ...
        PNS.confirm(confirm.title, confirm.content, function () {
            _this.submit();
        });
    }
        ...
        $form.ajaxSubmit({
            ...
            error: function (response) {
                ...
                try {
                    ...
                    if (response.status != 422 || ! error || ! PNS.helpers.isset(error, 'errors')) {
                        let json = response.responseJSON;
                        if (json && json.message) {
                            return PNS.error(json.message);
                        }

                        return PNS.error(response.status + ' ' + response.statusText);
                    }
                    error = error.errors;
    ...
    showError($form, column, errors) {
        ...
        if (! $field) {
            if (PNS.helpers.len(errors) && errors.length) {
                PNS.error(errors.join("  \n  "));
            }
            return;
        }
    ...
    isValueChanged($field, column) {
        return ! PNS.helpers.equal(this.originalValues[column], this.getFieldValue($field));
    }
