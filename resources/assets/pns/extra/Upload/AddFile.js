...
`).appendTo($li);
        ...
        try {
            image.once('load', function () {
                ...
                if (! _this.validateDimensions(file)) {
                    PNS.error(lang.trans('dimensions') || 'The image dimensions is invalid.');
        ...
        return function () {
            ...
            switch (index) {
                ...
                case 'delete':
                    ...
                    break;
                case 'preview':
                    PNS.helpers.previewImage($(this).parent().parent().find('img').attr('src'), null, file.name);
    ...
    validateDimensions(file) {
        let _this = this,
            ...
            height = file._info.height,
            isset = PNS.helpers.isset;

        // The image dimensions is invalid.
        if (! parent.isImage() || ! _this.isImage(file) || ! PNS.helpers.len(options.dimensions)) {
            return true;
        }
