...
<div class="btn-group dropdown  {{$selectAllName}}-btn" style="display:none;margin-right: 3px;z-index: 100">
    ...
    <ul class="dropdown-menu" role="menu">
        @foreach($actions as $action)
            @if ($action instanceof PNS\Admin\Grid\Tools\ActionDivider)
...
<script>
PNS.init('.{{ $parent->getRowName() }}-checkbox', function ($this) {
    $this.on('change', function () {
        var btn = $('.{{ $selectAllName }}-btn'), selected = PNS.grid.selectedRows('{{ $parent->getName() }}').length;
