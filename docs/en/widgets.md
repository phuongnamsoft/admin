...
`PNS\Admin\Widgets\Box` used to generate box components:

```php
use PNS\Admin\Widgets\Box;
...
`PNS\Admin\Widgets\Collapse` class used to generate folding components:
```php
use PNS\Admin\Widgets\Collapse;
...
`PNS\Admin\Widgets\Form` class is used to quickly build a form:
...
`PNS\Admin\Widgets\InfoBox` class is used to generate the information presentation block:

```php
use PNS\Admin\Widgets\InfoBox;
...
Refer to the section on the `InfoBox` in the `index()` method of the home page layout file [HomeController.php](https://github.com/phuongnamsoft/admin/blob/master/src/Console/stubs/HomeController.stub).

## Tab component

`PNS\Admin\Widgets\Tab` class is used to generate the tab components:
...
`PNS\Admin\Widgets\Table` class is used to generate tables:

```php
use PNS\Admin\Widgets\Table;
