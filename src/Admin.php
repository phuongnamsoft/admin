...
use Closure;
use PNS\Admin\Contracts\ExceptionHandler;
use PNS\Admin\Contracts\Repository;
use PNS\Admin\Exception\InvalidArgumentException;
use PNS\Admin\Http\Controllers\AuthController;
use PNS\Admin\Http\JsonResponse;
use PNS\Admin\Layout\Menu;
use PNS\Admin\Layout\Navbar;
use PNS\Admin\Layout\SectionManager;
use PNS\Admin\Repositories\EloquentRepository;
use PNS\Admin\Support\Composer;
use PNS\Admin\Support\Helper;
use PNS\Admin\Traits\HasAssets;
use PNS\Admin\Traits\HasHtml;
...
class Admin
{
    ...
    {
        return sprintf('PNS Admin <comment>version</comment> <info>%s</info>', static::VERSION);
    }
     ...
     * 配置.
     *
     * @return \PNS\Admin\Support\Setting
     */
     ...
     * 上下文管理.
     *
     * @return \PNS\Admin\Support\Context
     */
     ...
     * 翻译器.
     *
     * @return \PNS\Admin\Support\Translator
     */
     ...
     *
     * @param  string  $name
     * @return \PNS\Admin\Extend\Manager|\PNS\Admin\Extend\ServiceProvider|null
     */
    ...
    public static function routes()
    {
        ...
        if (config('admin.auth.enable', true)) {
            app('router')->group($attributes, function ($router) {
                ...
                $router->namespace('PNS\Admin\Http\Controllers')->group(function ($router) {
                    /* @var \Illuminate\Routing\Router $router */
                ...
                $router->resource('auth/extensions', 'PNS\Admin\Http\Controllers\ExtensionController', ['only' => ['index', 'store', 'update']]);
    ...
    public static function registerHelperRoutes()
    {
        ...
        app('router')->group($attributes, function ($router) {
            /* @var \Illuminate\Routing\Router $router */
            $router->get('helpers/scaffold', 'PNS\Admin\Http\Controllers\ScaffoldController@index');
            $router->post('helpers/scaffold', 'PNS\Admin\Http\Controllers\ScaffoldController@store');
            $router->post('helpers/scaffold/table', 'PNS\Admin\Http\Controllers\ScaffoldController@table');
            $router->get('helpers/icons', 'PNS\Admin\Http\Controllers\IconController@index');
        });
    }
}
